...
import lms_rozier_convers.core.tidying.BestRoomFitStrategy;
...
/**
 * 26/11/2015.
 * Tests the BestRoomFitStrategy
 */
public class BestRoomFitStrategyTest {
    /**
     * Tests whether an exception is thrown if the given room does not belong to the user library
     * @throws Exception
     */
    @SuppressWarnings("Duplicates")
    @Test (expected = ObjectNotFoundException.class)
    public void testTidyWithException() throws Exception {

        Library library = new Library(new BestRoomFitStrategy());
        ...
        Room room3 = new Room("Room3"); //Not added to the Library
        Book book = new Book(new Cuboid(5,2,7));
        library.getTidyingStrategy().tidy(book,room3);
    ...
    public void testTidy() throws Exception {
        ...
        Book book = new Book(new Cuboid(5, 2, 7));
        library.getTidyingStrategy().tidy(book, room1);
        assertTrue(book.getLocation().getShelf().getName().equals("shelf2"));  //Shelf2 has the biggest free space
    }
}