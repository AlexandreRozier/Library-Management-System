...
/**
 * 24/11/2015.
 * Tests the AnyFitStrategy
 */
@SuppressWarnings("ALL")
public class AnyFitStrategyTest {

    /**
     * Creates a library and sorts an item according to AnyFitStrategy
     * @throws Exception
     */
    @Test
    public void testTidy() throws Exception {
        ...
        Book book = new Book(new Cuboid(7,7,7));
        library.getTidyingStrategy().tidy(book);
        assertTrue(book.getLocation().getShelf().getName().equals("shelf7")); // Shelf 7 is the first one to have enough space to store book


        CD cd = new CD(new Cuboid(11,12,11));
